...
export default {
    ...
    methods: {
        ...
        let status= { id: 2, date: new Date().toISOString().slice(0, 10), email: this.employee.name}
        ...
        let repairedStatus = pothole.statuses.find(status => status.name == "Repaired")
        if(repairedStatus){
            return false
        ...
        handleClick(){
            ...
            this.$store.commit("SET_FILTER_FIELD", {fieldName:"employeeName", value:this.employee.name}) 
